% TUNEsub41() - Subroutine to comupute FFT spectrum and obtain
% Power Spectral Density (PSD), Sound Pressure Level (SPL) either 
% broadband (BB) or in decidecade bands (DD), and Zero-to-peak SPL.
% Called by 'TUNE' main.
...
% Zero-to-peak SPL is obtained by taking snapshot absolute max (pos/neg)
% then multiplying by an 'effective' scale factor corresponding to the
% frequency component in snapshot spectrum with highest amplitude.
% This approximates taking the scale factor of the peak half-cycle.
...
% and scalar values for BB and zero-to-peak SPL.
...
% Debug mode requires key strokes between plots, added annotations in
% plot figures as a reminder.
%
...
% 4.1 - Added option for zero-to-peak SPL.
%
% COPYRIGHT
...
%
% VERSION 4.1 | 03-sep-2025 

function [bbpxxavg,ddpxxavg,fbpxxavg,zerotopeak] = ...
                TUNEsub41(Snap,Win,BBSF,DDSF,DDmat,Param)
...
PROCSPLP = Param.procsplp; % peak SPL
PEAKFMIN = Param.peakfmin; % min. f to detect spectrum peak in Hz

% plot titles and labels (to be updated for PSD and decidecade SPL)
...
if PROCSPLD
    figSPLD = Param.figspld; % decidecade SPL figure handle
end
...
%        Y = Snap.Y(length(Snap.Y) - Param.nf + 1:length(Snap.Y));
    ...
    if VERB == 2
        % figure(figWAV);
        set(0,'CurrentFigure',figWAV); % draw subint on waveform figure
        if islastsubint
            clf % need this to get rid of annotation box
            plot(1:Param.nf,Ynoslide) % show zero padded subint
            annotation("textbox",[0.4 0.1 0.2 0.2], ...
                "String","Zeropad - hit enter...","FitBoxToText","on");
        ...
        plot(1:Param.nf,Y)
        annotation("textbox",[0.4 0.1 0.2 0.2], ...
            "String","Hit enter to continue...","FitBoxToText","on");
        pause % hit any key
   end
    %%%%%%%%%%%%%% END DEBUG
    ...
    if VERB > 0
        fprintf(' max = %.1f dB',max(bbpxxdb)); % disp. max if verbose
    end
...
if PROCSPLB || PROCPSDB || PROCSPLP  % broadband or peak SPL or PSD
...
if PROCSPLP  % zero-to-peak SPL option, added in main v. 6.11
    % Take max amplitude in snap, multiply by calibration scale factor
    %  corresponding to spectrum peak starting from PEAKFMIN
    peakY = max(abs(Snap.Y)); % snap max positive or negative peak
    fidxvect = find(bbf >= PEAKFMIN); % first element is the index of
                    %   f vector element corresponding to PEAKFMIN
    pxxtemp = bbpxxavg; % copy power spectrum to temp. array
    pxxtemp(1:fidxvect(1)) = 0; % set to 0 components below min. f
    [~ , peakIdx] = findpeaks(pxxtemp,"SortStr","descend"); % peaks array
    peakSF = BBSF(peakIdx(1)); % SF at peak f
    zerotopeak = peakY * peakSF; % resulting value in pascal
    if VERB > 0
        peakF = bbf(peakIdx(1)); % f value of peak index
        fprintf('\nPeak Y = %.3e, f = %.1f Hz, SF = %.1f Pa', ...
                    peakY,peakF,peakSF);
    end
else
    zerotopeak = []; % return void if not selected
end
