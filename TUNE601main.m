...
%	 (was done by taking wave rms and mean SF); decidecade now is now
...
for wFileN = 3:length(wFiles)
    ...
    for snapN = 1 : nSnap
        ...
        if PROCPSDB || PROCSPLD || PROCSPLB
            [curPsdB,curSplD,curSplB] = ...
                    evalPSDBBDD40(Snap,FFTW,CalSF,DDCalSF,DDmat,Param);
    ...
    if PROCSPLB
        % plot is already updated on each snap
    end
